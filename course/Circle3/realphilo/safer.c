...
static void handle_mutex_error(int status, t_opcode opcode)
{
    ...
    if (EINVAL == status && (LOCK == opcode || UNLOCK == opcode))
        ...
        show_error("A dead lock would occur if the thread blocked waiting for mutex");
    else if (EPERM == status)
        show_error("The current thread does not own the mutex.");
    else if (ENOMEM == status)
        show_error("There is not enough memory to acquire a lock. This errno will only occur in the private path.");
    else if (EPERM == status)
        show_error("The current thread does not own the mutex.");
